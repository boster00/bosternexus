 ...
 * BooksEstimateEntity
 * 
 * Entity definition for Zoho Books Estimates (Quotations).
 * Defines field mappings, transformations, and table structure.
 */

export class BooksEstimateEntity {
  static tableName = 'zoho_books_estimates';
  static zohoIdField = 'estimate_id';
  static apiEndpoint = '/estimates';
  ...
  static fieldMappings = {
    'estimate_id': { dbColumn: 'zoho_id', required: true, transform: null },
    'estimate_number': { dbColumn: 'estimate_number', required: false, transform: (v) => v || null },
    'date': { dbColumn: 'date', required: false, transform: (v) => v || null },
    'expiry_date': { dbColumn: 'expiry_date', required: false, transform: (v) => v || null },
  ...
  static transformToDbRecord(zohoEstimate) {
    const record = {
    ...
    for (const [zohoField, mapping] of Object.entries(this.fieldMappings)) {
      ...
      const zohoValue = zohoEstimate[zohoField];
  ...
  /**
   * Extract estimate data from Zoho API response
   ...
   * @returns {Array} Array of estimate objects (always returns array, even for single records)
   */
  static extractFromResponse(response) {
    ...
    // /estimates -> plural: 'estimates', singular: 'estimate'
    const pluralName = this.apiEndpoint.replace('/', ''); // 'estimates'
    const singularName = pluralName.slice(0, -1); // 'estimate' (remove 's')
